...
class Galaxy:
    ...
    def clean_spectrum(
        ...
        Then use ppxf and auxillary functions to 'clean' the spectrum. Ie:
          ...
          - the bad_pix keyword provides an option for used input pixels to specifically exclude.
            these pixels are excluded _in addition to_ any others that are found to be bad.
        """
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Get all the needed parameters from set_up_ppxf
        [loglam_gal, var, mask, goodpix, ns_clean, logflux_temp, loglam_temp, dv, velscale] = (
            sph.set_up_ppxf(
        ...
        # Check spectrum for NaNs
        # nan_pix = np.where(np.isnan(self.logflux_gal))
        # mask[nan_pix] = False
        ...
        diff = logflux_gal / np.nanmedian(logflux_gal) - self.pp1.bestfit / np.median(
            self.pp1.bestfit
        )
        # define limit of acceptable difference = 5* Root-Median-Squared
        diff_val = 5.0 * math.sqrt(np.nanmedian(diff**2))
        ...
        cond4 = (loglam_gal * (1 + self.z) > 5565) & (loglam_gal * (1 + self.z) < 5590)
        ...
        if len(m) > 0:
            temp[m] = (
                self.pp1.bestfit[m] * np.nanmedian(logflux_gal) / np.nanmedian(self.pp1.bestfit)
            )
        ...
        linlam_gal = np.linspace(loglam_gal[0], loglam_gal[-1], len(loglam_gal))
        self.galaxy_final = np.interp(linlam_gal, loglam_gal, self.log_galaxy_final)
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        vel_disp = self.pp1.sol[1].clip(min=0.0, max=300.0)

        # Need to interpolate the logarithmically binned FWHM_inst to a linear
        # scale.
        FWHM_inst_lin = np.interp(self.linlam_gal_rest, loglam_gal, self.FWHM_inst[mask])
        ...
        self.sig_inst = (FWHM_inst_lin / math.sqrt(8 * np.log(2))) / (self.extra_redshift + 1)
    ...
    def measure_lick_indices(
        ...
        assert hasattr(self, "galaxy_final") is True, "clean the spectrum" "first!"
        if index_table is None:
            # get the path to this file
            data_dir = files("StellarProperties") / "data" / "Lick_and_Models"
            index_table = data_dir / "Lick_index_definitions.dat"
        ...
        for i in range(len(indices)):
            res[i] = index_defs[np.where(index_defs["Index"] == indices[i])[0]]["Res"][0]
        ...
        def measure_index(j):
            ...
            ind_range = [index_defs[row]["BC_lower"][0], index_defs[row]["RC_upper"][0]]

            if self.linlam_gal_rest[0] > ind_range[0] or self.linlam_gal_rest[-1] < ind_range[-1]:
                if info:
                    print(f"{indices[j]} couldn't be measured: outside " "spectral range")
            ...
            relevant_inds = np.where(
                ...
                & (self.linlam_gal_rest < index_defs[row]["I_upper"][0])
            )[0]
            ...
            if broaden is True:
                if sig_relevant < sig_ind:
                    ...
                    broadening = np.sqrt((sig_ind**2 - sig_relevant**2).clip(0)) / dlambda
                    # broaden spectrum
                    lick_res_spectrum = pu.gaussian_filter1d(flux, broadening)
                    # broaden noise
                    lick_res_var = pu.gaussian_filter1d(noise**2, broadening)
                    index_j = sph.measure_ind(
                        wavelength, lick_res_spectrum, indices[j], index_defs
                    )
                    err_j = sph.mc_ind_errors(
                        wavelength, lick_res_spectrum, lick_res_var, indices[j], index_defs
                    )
                ...
                # dispersion
                    ...
                    if info:
                        print("spectrum resolution worse than index" "resolution")
                    # vdisp is amount of dispersion that needs to be corrected
                    # for (in km/s). NOTE:
                    # sigma [km/s] =  sigma [A] * c [km/s] / wavelength [A]
                    vdisp_squish = (
                        np.sqrt(sig_relevant**2 - sig_ind**2)
                        * c
                        / np.median(self.linlam_gal_rest[relevant_inds])
                    )
                    broadening = np.nan
                    lick_res_spectrum = flux
                    lick_res_var = noise**2
                    width = sph.measure_ind(wavelength, lick_res_spectrum, indices[j], index_defs)
                    err_j = sph.mc_ind_errors(
                        wavelength, lick_res_spectrum, lick_res_var, indices[j], index_defs
                    )
                    # Now correct width calculated above for exessive velocity
                    # dispersion broadening
                    index_j = sph.vdisp_correct(vdisp_squish, width, indices[j])

            elif broaden is False:
                broadening = 0
                lick_res_spectrum = np.copy(flux)
                lick_res_var = noise**2
                index_j = sph.measure_ind(wavelength, lick_res_spectrum, indices[j], index_defs)
                err_j = sph.mc_ind_errors(
                    wavelength, lick_res_spectrum, lick_res_var, indices[j], index_defs
                )
        ...
        self.lick_indices = dict(
            [(indices[i], [results[i, 0], results[i, 1]]) for i in range(len(indices))]
        )

        # =====================================================================
        # =====================================================================

    def measure_sp_parameters(self, model="tmj", n_models="all", model_file=None, params=None):
        ...
        assert hasattr(self, "lick_indices") is True, "you havent measured " "lick indices yet"
        ...
        if model == "s07" and model_file is None:
            model_file = data_dir / (
                "alpha_models_ti_schiavon_reinterpolated_" "log_age_nolick.fits"
            )
            params = ["age", "Z_H", "alpha_Fe"]

        if model == "tmj" and model_file is None:
            model_file = data_dir / "TMB_TMK_interpolated_alpha_model.fits"
            params = ["age", "Z_H", "alpha_Fe"]

        if model == "miles" and model_file is None:
            model_file = data_dir / ("Miles_Indicies_BaSTI_Chabrier_" "interpolated.fits")
        ...
        def chi2_val(index, extra_args):
            ...
            for j in range(n_models):
                ...
                # index measure
                chi2_arr[j] = (
                    (index_dict[index][0] - ssp_table[index][j]) / index_dict[index][1]
                ) ** 2
            return chi2_arr

        extra_args = (len(ssp_table), self.lick_indices, ssp_table)
        chi2_2Darr = np.array([chi2_val(ind, extra_args) for ind in av_indices])
        ...
        sp_dict = dict(
            [
                (p, [ssp_table[p][w][0], np.max(ssp_table[p][v]), np.min(ssp_table[p][v])])
                for p in params
            ]
        )
    ...
    def full_spectral_fitting(
        ...
        **kwargs,
        ...
        if gas:
            ...
            moments = [moments for _ in range(n_components)]
        ...
        setattr(getattr(self, "pp" + tag), "weighting", models.weighting)

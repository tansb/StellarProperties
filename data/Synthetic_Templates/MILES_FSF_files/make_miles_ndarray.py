...
filenames          = np.array(glob.glob('../MILES_SSP_files/MILES_BASTI_CH_baseFe/Mch*.fits'))
...
# 4300 = npix in each spectrum
star_temps = np.zeros((4300, n_ages, n_mets))
...
for a, age in enumerate(unique_ages):
	for m, metal in enumerate(unique_mets):
		...
		star_temps[:, a, m] = spec #/ np.mean(spec)
		#age_grid1[a, m] = age
		#met_grid1[a, m] = metal

# Normalise all the tempaltes as a whole to have a median of 1 (not individually normalised; which
# would make them light weighted)
star_temps /= np.median(star_temps)

# save everything
np.save('../MILES_SSP_files/MILES_all_data_3D', star_temps)
np.save('../MILES_SSP_files/MILES_age_grid',    age_grid)
np.save('../MILES_SSP_files/MILES_met_grid',    met_grid)
